...
```
POST   /api/v1/auth/token/                      ← Login (email/username + password) → JWT tokens
POST   /api/v1/auth/google/login/               ← Google login (role-aware)
POST   /api/v1/auth/google/signup/              ← Google signup (role-aware)
POST   /api/v1/auth/magic-link/request/         ← Request email magic link
POST   /api/v1/auth/magic-link/verify/          ← Verify magic link and issue JWT
POST   /api/v1/auth/username/check/             ← Username availability check
...
GET    /api/v1/merchant/onboarding/business-types/
...
```
Password Login Flow:
  1. POST /auth/token/ with {identifier, password, role}
  2. Backend resolves identifier as email OR username
  3. On success, returns access_token (15min) + refresh_token (7d)

Google Login / Signup Flow:
  1. POST /auth/google/login/ or /auth/google/signup/ with {id_token, role}
  2. Backend validates Google ID token and enforces role mismatch checks
  3. Role-aware auto-provision is allowed for merchant login/signup when account does not exist
  4. JWT pair is issued on success

Magic-Link Flow (Signup and fallback flows):
  1. POST /auth/magic-link/request/ with {email, role, redirect_url}
  2. User receives signed link and lands on frontend callback route
  3. POST /auth/magic-link/verify/ with {email, token, role}
  4. JWT pair is issued on success

Merchant Web Contract:
  - /login: email-or-username + password, plus Google button
  - /register: magic-link request + Google button
  - /auth/callback: dedicated magic-link verification route
...
Storage structure:
  ...
  ├── user/{user_id}/
  │   └── files/
  │       ├── valid_id/front/{filename}
  │       ├── valid_id/selfie_with_id/{filename}
  │       └── business_documents/primary/{filename}
  ├── rider/{rider_id}/
  │   └── files/
  │       ├── valid_id/front/{filename}
  │       ├── valid_id/selfie_with_id/{filename}
  ...
  │   ├── files/
  │   │   ├── valid_id/front/{filename}
  │   │   ├── valid_id/back/{filename}
  │   │   ├── valid_id/selfie_with_id/{filename}
  │   │   ├── business_permits/barangay/{filename}
  │   │   ├── business_permits/mayors_permit/{filename}
  │   │   ├── business_registration/dti_or_sec/{filename}
  │   │   ├── tax_documents/bir_2303/{filename}
  │   │   └── other_documents/{filename}
  │   ├── profile/images/{filename}
  │   └── stores/{store_id}/
  │       ├── assets/logo/images/{filename}
  │       ├── assets/banner/images/{filename}
  │       └── products/{store_type}/media/
  │           ├── images/{filename}
  │           └── videos/{filename}
  └── communications/chat/threads/{thread_id}/attachments/{role}/{account_id}/
      ├── images/{filename}
      └── files/{filename}
```

Mandatory Taxonomy Rules:
- All upload writes must go through centralized helpers in `apps/core/storage.py`; ad hoc path strings in views/services are disallowed.
- Owner-first root is required for all role-bound uploads: `user/{id}`, `rider/{id}`, `merchant/{id}`, `admin/{id}`, `superadmin/{id}`.
- Product media must be categorized under `media/images` or `media/videos`.
- Chat attachments must include both thread scope and sender scope (`{role}/{account_id}`) for auditability.
- Existing legacy paths are backward-compatible at read-time, but all new uploads must follow this structure.
...
## 13. AUTH + MERCHANT ONBOARDING UPDATE (APRIL 2026)

### Global Authentication (All Roles)

- Primary auth: Google OAuth (`/api/v1/auth/google/login/`)
- Fallback auth: email magic link (`/api/v1/auth/magic-link/request/`, `/api/v1/auth/magic-link/verify/`)
- Username availability endpoint: `/api/v1/auth/username/check/`
- Deprecated path: password-based login should no longer be used by web apps.

### Identity and Role Rules

- Email is the identity merge key across Google and magic-link sign-in.
- Role mismatch for existing email is blocked with explicit conflict response.
- Magic-link verification can auto-provision users with role-specific defaults:
  - Merchant: `status=PENDING`, `wizard_completed=false`
  - Other roles: `status=APPROVED`, `wizard_completed=true`

### Merchant Onboarding (5-Step Normalized Flow)

- Step 1: Profile
- Step 2: Business
- Step 3: Location
- Step 4: Documents
- Step 5: OTP verify + legal acceptance + submit

Persistence model:

- Merchant profile holds approved operational fields used by commerce modules.
- Onboarding drafts and verification state are stored in normalized onboarding tables.
- Final submit maps approved values from onboarding tables into `MerchantProfile`.
...
Merchant gating rules:

- `wizard_completed=false` => force onboarding route
- `status=PENDING` => pending-only screen, dashboard locked
- resubmission allowed only when admin explicitly enables `can_resubmit`

### Frontend Notes

- Google sign-in button locale is forced to English.
- Merchant map input uses provider toggle (`NEXT_PUBLIC_MAP_PROVIDER`) with Leaflet-first implementation.
- Document and selfie capture uses lightweight browser capability checks (no heavy AI dependency).
