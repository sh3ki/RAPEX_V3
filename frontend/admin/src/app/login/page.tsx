    ...
    async (idToken: string) => {
      ...
      try {
        ...
        if (code === 'google_signup_required') {
          setError('No admin account is linked to this Google email yet. Please sign up first.');
        } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Sign in to RAPEX Admin Panel</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                ...
                placeholder="admin@rapex.ph" required />
            </div>
            <div>
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            New admin? <Link href="/register" className="text-primary-500 font-medium hover:underline">Sign up with Google</Link>
          </p>
        </div>
      </div>
    </div>
