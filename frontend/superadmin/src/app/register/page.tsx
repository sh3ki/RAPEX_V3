...
export default function RegisterPage() {
  ...
  const handleMagicLink = async (event: React.FormEvent) => {
    ...
    setInfo('');
    try {
      const result = await requestMagicLink(email, 'SUPERADMIN', `${window.location.origin}/register`);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Create SuperAdmin Access</h1>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 space-y-5">
          ...
          <p className="text-center text-sm text-gray-500 mt-2">
            Already have access? <Link href="/login" className="text-primary-500 font-medium hover:underline">Open superadmin login</Link>
          </p>
        </div>
      </div>
    </div>
